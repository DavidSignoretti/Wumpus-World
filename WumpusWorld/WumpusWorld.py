...
class Agent:
    ...
    def __init__(self):
        ...
        :parameter
            ...
            scream: bool False : did the agent hear the wumpus scream
...
class Board:
    ...
    def __init__(self, width: int, height: int, pits: float):
        ...
        self.probwumpus: po.BayesianNetwork = po.BayesianNetwork("Wumpus")
        self.probpits: po.BayesianNetwork = po.BayesianNetwork("Pits")
        ...
        for _y in range(0, self.height):
            for _x in range(0, self.width):
                if _x == 0 and _y == 0:
                    ...
                    _s: str = str(_x) + str(_y)
                    _d.update({_s: (1.0/((self.height*self.width)-1))})

        _c = [[True, False, False, False, 1.0],
             [False, True, False, False, 1.0],
             [False, False, True, False, 1.0],
             [False, False, False, True, 1.0],
             [False, False, False, False, 0]]

        _wumpusdd = po.DiscreteDistribution(_d)
        _wumpuscd = po.ConditionalProbabilityTable(_c, [_wumpusdd])

        _wumpusddnode = po.Node(_wumpusdd, name="_wumpusddnode")
        _wumpuscdnode = po.Node(_wumpuscd, name="_wumpuscdnode")

        self.probwumpus.add_states(_wumpusddnode, _wumpuscdnode)
        self.probwumpus.add_edge(_wumpusddnode,_wumpuscdnode)
        self.probwumpus.bake()
        ...
        for _y in range(0, height):
            for _x in range(0, width):
                if _x == 0 and _y == 0:
                    ...
                    _s: str = str(_x) + str(_y)
                    _d.update({_s: self.pits})

        _pits = po.DiscreteDistribution(_d)
